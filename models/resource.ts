...
const Resource = {
  ...
  clone(this: DecoratedResource, store: any) {
    return async () => {
      ...
      const neu = (await decorate(
        JSON.parse(json),
        store
      )) as DecoratedResource;

      Object.defineProperty(neu, "__original", {
        configurable: false,
        enumerable: false,
        value: json,
      });
      ...
      try {
        ...
        if (forNew && loadAfterSave(this.type)) {
          if (store.map[this.id]) {
            ...
            console.error(
              `Warning: Newly created ${this.type} ${this.id} is already in the store`
            );
  ...
  nameDisplay(this: DecoratedResource) {
    return computed(
      (): string =>
        (this.spec?.displayName as string) || this.metadata?.name || this.id
    );
  },
  ...
  nameSort(this: DecoratedResource) {
    return computed(() => {
      ...
      )}/${sortableNumericSuffix(name).toLowerCase()}`;
    });
  },
  ...
  isActive(this: DecoratedResource) {
    return computed(() => {
      return (
        ["active", "running"].includes(unref(this.state)) &&
  ...
  isRemoving(this: DecoratedResource) {
    return computed(() => {
      return (
        unref(this.state) === "removing" && this.metadata.state?.transitioning
      );
    });
  },
  ...
  stateColor(this: DecoratedResource) {
    return computed(() => {
      ...
      return colorForState(
        state,
        this.metadata.state?.error || false,
        this.metadata.state?.transitioning || false
      );
    });
  },
  ...
  isCondition(this: DecoratedResource) {
    return (condition: string, withStatus: string | null = "True") => {
      ...
      return (
        (entry.status || "").toLowerCase() === `${withStatus}`.toLowerCase()
      );
    };
  },
  ...
  serverAction(this: DecoratedResource, store: SteveStoreType) {
    return (
      ...
      if (!opt.url) {
        throw new Error(
          `Unknown action ${actionName} on ${this.type} ${this.id}`
        );
      }
  ...
  canUpdate(this: DecoratedResource) {
    return computed(
      () => <boolean>editable(this.type) && this.hasLink("update")
    );
  },
      ...
      const all: IAction[] = [
        ...
        },
        {
          ...
          confirm: confirmRemove
            ? () => `Are you sure you want to remove '${this.nameDisplay}'?`
            : false,
          label: "Remove",
          icon: "trash-can",
          bulkable: true,
  ...
  remove(this: DecoratedResource, store: SteveStoreType) {
    return async (opt: IRequestOpt = {}) => {
      ...
      const res = await store.request(opt);
