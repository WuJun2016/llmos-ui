...
export function validateLength(val, field, displayKey, getters, errors = []) {
  ...
  if (!nullable && required) {
    if ((typeof val === "object" && isEmpty(val)) || (!val && val !== 0)) {
      errors.push(
        getters["i18n/t"]("validation.required", { key: displayKey })
      );
  ...
  const lengthKey =
    type.indexOf("array[") === 0 ? "arrayLength" : "stringLength";
  ...
  if (min && max) {
    if (len < min || len > max) {
      if (min === max) {
        errors.push(
          getters["i18n/t"](`validation.${lengthKey}.exactly`, {
            key: displayKey,
            count: min,
          })
        );
        ...
        errors.push(
          getters["i18n/t"](`validation.${lengthKey}.between`, {
            key: displayKey,
            min,
            max,
          })
    ...
    errors.push(
      getters["i18n/t"](`validation.${lengthKey}.min`, {
        key: displayKey,
        count: min,
      })
    );
    ...
    errors.push(
      getters["i18n/t"](`validation.${lengthKey}.max`, {
        key: displayKey,
        count: max,
      })
    );
  ...
  if (val !== null && min && max) {
    if (val < min || val > max) {
      if (min === max) {
        errors.push(
          getters["i18n/t"]("validation.number.exactly", {
            key: displayKey,
            val: max,
          })
        );
        ...
        errors.push(
          getters["i18n/t"]("validation.number.between", {
            key: displayKey,
            min,
            max,
          })
    ...
    errors.push(
      getters["i18n/t"]("validation.number.min", { key: displayKey, val: min })
    );
  } else if (max && val > max) {
    errors.push(
      getters["i18n/t"]("validation.number.max", { key: displayKey, val: max })
    );
  }
...
export function validateChars(val, field, displayKey, getters, errors = []) {
  const { validChars, invalidChars } = field;
  const test = [];
  ...
  if (test.length) {
    ...
    if (match) {
      ...
      errors.push(
        getters["i18n/t"]("validation.chars", {
          key: displayKey,
          count: match.length,
          chars: match.join(" "),
        })
...
export function validateHostname(val, displayKey, getters, opts, errors = []) {
  ...
  const { max = 253, restricted = false } = opts;

  // Hostname can not start with a dot
  if (val.slice(0, 1) === ".") {
    errors.push(
      getters["i18n/t"]("validation.dns.hostname.startDot", { key: displayKey })
    );
  }
  ...
  if (restricted && val.length > 1 && val.slice(-1) === ".") {
    errors.push(
      getters["i18n/t"]("validation.dns.hostname.endDot", { key: displayKey })
    );
  }
  ...
  if (val.length === 0) {
    errors.push(
      getters["i18n/t"]("validation.dns.hostname.empty", { key: displayKey })
    );
  }
  ...
  if (val.length > max) {
    errors.push(
      getters["i18n/t"]("validation.dns.hostname.tooLong", {
        key: displayKey,
        max,
      })
    );
...
export function validateDnsLabel(
  ...
  if (restricted && label.slice(0, 1).match(/[0-9]/)) {
    errors.push(
      getters["i18n/t"](`validation.dns.${errorKey}.startNumber`, {
        key: displayKey,
      })
    );
  }
  ...
  if (label.slice(0, 1) === "-") {
    errors.push(
      getters["i18n/t"](`validation.dns.${errorKey}.startHyphen`, {
        key: displayKey,
      })
    );
  }
  ...
  if (label.slice(-1) === "-") {
    errors.push(
      getters["i18n/t"](`validation.dns.${errorKey}.endHyphen`, {
        key: displayKey,
      })
    );
  }
  ...
  if (
    ...
    errors.push(
      getters["i18n/t"](`validation.dns.doubleHyphen`, { key: displayKey })
    );
  }
  ...
  if (label.length < min) {
    errors.push(
      getters["i18n/t"](`validation.dns.${errorKey}.emptyLabel`, {
        key: displayKey,
        min,
      })
    );
    ...
    errors.push(
      getters["i18n/t"](`validation.dns.${errorKey}.tooLongLabel`, {
        key: displayKey,
        max,
      })
    );
...
export function validateDnsLikeTypes(
  val,
  type,
  displayKey,
  getters,
