...
export function camelize(str: string): string {
  return str
    .split(camelizeRegex)
    .map((x, i) => (i === 0 ? lcFirst(x) : ucFirst(x)))
    .join("");
}
...
export function matchesSomeRegex(
  str: string,
...
export function strPad(
  ...
  const padStr = Array.from({ length: neededLen })
    .join(padChars)
    .substring(0, neededLen);
...
export async function safeConcatNameWithOptions(
  maxLength: number,
  separator: string,
  ...parts: string[]
): Promise<string> {
  ...
  out = out.substring(0, maxLength - separator.length - hashLength);

  do {
    const last = out.substring(out.length - 1);

    ok = (last >= "a" && last <= "z") || (last >= "0" && last <= "9");
  ...
  out += separator + hash.substring(0, hashLength);
